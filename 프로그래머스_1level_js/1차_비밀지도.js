 ...
 * 
 * 지도는 한 변의 길이가 n인 정사각형 배열 형태로, 각 칸은 "공백"(" ") 
 * 또는 "벽"("#") 두 종류로 이루어져 있다.
...
/**
 * newArr1 과 newArr2 변수에 빈 배열을 업로드 해주고 
 * 첫번째 for 루프에서는 arr1의 각 요소를 이진수로 변환하여 newArr1에 추가 한다.
 * 
 * 두번째 for는 arr2의 각 요소를 2진수로 변환후 newArr2에 추가한다.
 * 
 * 세번째 for은 이중 for문으로 제작하여 두개의 2진수 배열을 비교하여 새로운 배열 answer
 * 을 만든다. 이때 if else 문에서 인덱스의 값이 모두 0이면 공백을 아니면 #을 추가한다.
 */